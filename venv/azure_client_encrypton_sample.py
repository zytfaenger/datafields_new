...
STORAGE_URL = "https://easyelblob.blob.core.windows.net"
...
class KeyWrapper:
    ...
    def __init__(self, kek, credential):
        self.algorithm = KeyWrapAlgorithm.rsa_oaep
        self.kek = kek
        self.kid = kek.id
        self.client = CryptographyClient(kek, credential)

    def wrap_key(self, key):
        if self.algorithm != KeyWrapAlgorithm.rsa_oaep:
...
container_name = make_resource_name('fscontainer')
blob_name = make_resource_name('fsblob')

container_client = storage_client.get_container_client(container_name)
container_client.key_encryption_key = crypo_client.wrap_key(EncryptionAlgorithm.rsa_oaep,)
